					        ...
					        <%--隐藏表单--提交 --%>
					        	<div class="submit-confirm">
							 	...
							 	<div class="delete-confirm">
									<span>
										<button onclick="formDelete()">删除</button>
									</span>
								</div>

								<%---回复-- 删除表单 --%>
								<form action="/iDouBan/ArticleShowServlet?pre_method=delete_Comment&method=delete_Comment&article_id=${requestScope.article.articleId}" id="deleteSubmit" name="reply_form" method="post" >
									<%--隐藏表单--评论的id --%>
									<input type="hidden" name="comment_id" id="delete_comment_id">
									<!-- 被回复的回复区中的用户id -->
									<input type="hidden" name="reply_to_use_id" id="delete_to_use_id">
								</form>
						 </div>
...
/***********************************************/
						...
						if( obj.objects[i].starStatus == 0 )
							 {
								...
								'							<a href="#delete-confirm" class=\"reply_a\" id=\"'+obj.objects[i].commentId+'\"  style=\"color : grey;float: right;margin: 9px 2px;\">删除</a>'+
	...
	function deleteSubmit(){
		...
		alert(toCommentId);
		document.getElementById("delete_comment_id").value = toCommentId;
		alert(document.getElementById("delete_comment_id").value);
	}
