...
public class ArticleShowDaoImpl implements ArticleShowDao
{
	...
	@Override
	public boolean replyDelete(ArticleReply articleReply) {
		boolean execute = false;
		try {
			con = dataSource.getConnection();
			//根据评论查找的sql语句
			String sql = "DELETE from a_comment where comment_id = ?";
			stmt = con.prepareStatement(sql);
			stmt.setInt( 1, articleReply.getCommentId() );
	...
	@Override
	public boolean updateReply(int articleId) {
		boolean execute = false;
		try {
			con = dataSource.getConnection();
			//根据评论查找的sql语句
			String sql = "update a_article set comment = comment-1 where article_id = ?";
